...
export default function SinglePost() {
    ...
    if (isError || !post) return (
        <div className="min-h-screen flex flex-col items-center justify-center bg-[#FAFAFA] text-[#666666]">
            <h2 className="text-2xl font-bold mb-4">Post not found</h2>
            <button onClick={() => navigate('/posts')} className="px-6 py-2 bg-gradient-to-r from-[#D64A26] to-[#F1795D] hover:from-[#c24220] hover:to-[#e36345] text-white rounded-xl font-bold uppercase tracking-widest text-xs shadow-md transition-colors">Go to Feed</button>
        </div>
    );
    ...
    return (
        <div className="min-h-screen py-16 px-4 sm:px-6 bg-[#FAFAFA] font-sans">
            <div className="max-w-3xl mx-auto">
                ...
                <div className="bg-white/95 backdrop-blur-sm rounded-2xl border border-slate-100 shadow-sm transition-all duration-300 hover:shadow-[0_20px_40px_rgba(0,0,0,0.06)] overflow-hidden">
                    <div className="px-6 md:px-8 pt-6 pb-4 flex items-start justify-between gap-4">
                        <div className="flex items-center gap-4">
                            <div className="w-14 h-14 rounded-full bg-slate-100 text-[#D64A26] flex items-center justify-center font-bold text-2xl ring-1 ring-slate-200/60">
                                {post.author?.full_name?.charAt(0).toUpperCase() || 'A'}
                            </div>
                            <div>
                                <h3 className="font-bold text-[#222222] text-xl leading-tight">{post.author?.full_name || 'CRC Admin'}</h3>
                                <p className="text-[#666666] text-[0.9rem] font-medium mt-1">{date}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="px-4 md:px-6 py-4 border-t border-slate-50 flex items-center justify-between">
                        ...
                        <button
                            onClick={handleShare}
                            className="flex items-center gap-2 px-5 py-2.5 rounded-full text-slate-400 hover:text-[#666666] hover:bg-slate-50 font-medium transition-all"
                        >
                            <ShareIcon />
                            Share
                        </button>
